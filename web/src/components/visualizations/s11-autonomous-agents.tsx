...
const FSM_STATES: { id: Phase; labelKey: string; angle: number }[] = [
  { id: "idle", labelKey: "s11_phase_idle", angle: -Math.PI / 2 },
  { id: "poll", labelKey: "s11_phase_poll", angle: 0 },
  { id: "claim", labelKey: "s11_phase_claim", angle: Math.PI / 2 },
  { id: "work", labelKey: "s11_phase_work", angle: Math.PI },
];
...
const INITIAL_TASKS: TaskRow[] = [
  { id: "T1", nameKey: "s11_task1", status: "unclaimed", owner: "-" },
  { id: "T2", nameKey: "s11_task2", status: "unclaimed", owner: "-" },
  { id: "T3", nameKey: "s11_task3", status: "unclaimed", owner: "-" },
  { id: "T4", nameKey: "s11_task4", status: "unclaimed", owner: "-" },
];
...
export default function AutonomousAgents({ title }: { title?: string }) {
  ...
  const steps = Array.from({ length: 8 }, (_, i) => ({
    title: t(`s11_step${i}_title`),
    desc: t(`s11_step${i}_desc`),
  }));
      ...
      <div className="rounded-lg border border-zinc-200 bg-white p-4 dark:border-zinc-700 dark:bg-zinc-900 min-h-[500px]">
        <div className="flex flex-col lg:flex-row gap-4">
          ...
          <div className="flex-1">
            <div className="text-xs font-medium text-zinc-500 dark:text-zinc-400 mb-2">{t("s11_spatial_view")}</div>
            <svg viewBox="0 0 280 240" className="w-full">
              ...
              >
                {tasks.filter((t) => t.status === "unclaimed").length} {t("s11_unclaimed")}
              </text>
              <text x={BOARD_CX} y={BOARD_CY + 14} textAnchor="middle" fontSize={6} fontFamily="monospace"
                fill="#10b981"
              >
                {tasks.filter((t) => t.status === "complete").length} {t("s11_complete")}
              </text>
              ...
              <table className="w-full text-[10px]">
                <thead>
                  <tr className="bg-zinc-50 dark:bg-zinc-800">
                    <th className="px-2 py-1 text-left font-medium text-zinc-500 dark:text-zinc-400">{t("s11_table_task")}</th>
                    <th className="px-2 py-1 text-left font-medium text-zinc-500 dark:text-zinc-400">{t("s11_table_status")}</th>
                    <th className="px-2 py-1 text-left font-medium text-zinc-500 dark:text-zinc-400">{t("s11_table_owner")}</th>
                  </tr>
                </thead>
                <tbody>
                  {tasks.map((task) => (
                    <tr key={task.id} className="border-t border-zinc-100 dark:border-zinc-800">
                      <td className="px-2 py-1 font-mono text-zinc-700 dark:text-zinc-300">{t(task.nameKey)}</td>
          ...
          <div className="flex-1">
            <div className="text-xs font-medium text-zinc-500 dark:text-zinc-400 mb-2">{t("s11_fsm_cycle")}</div>
            ...
            <div className="mt-2 flex flex-wrap gap-3 justify-center">
              {FSM_STATES.map((s) => (
                <div key={s.id} className="flex items-center gap-1">
                  ...
                  <span className="text-[10px] font-mono text-zinc-500 dark:text-zinc-400">{t(s.labelKey)}</span>
                </div>
              ))}
            </div>
          </div>
