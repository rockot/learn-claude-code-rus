...
export default function TeamProtocols({ title }: { title?: string }) {
  ...
  const shutdownSteps = Array.from({ length: 4 }, (_, i) => ({
    title: t(`s10_shutdown_step${i}_title`),
    desc: t(`s10_shutdown_step${i}_desc`),
  }));
  const planSteps = Array.from({ length: 3 }, (_, i) => ({
    title: t(`s10_plan_step${i}_title`),
    desc: t(`s10_plan_step${i}_desc`),
  }));
  const totalSteps = protocol === "shutdown" ? shutdownSteps.length : planSteps.length;
  const steps = protocol === "shutdown" ? shutdownSteps : planSteps;
  ...
  const leftLabel = t("s10_left_label");
  const rightLabel = t("s10_right_label");

  return (
    <section className="space-y-4">
      <h2 className="text-xl font-semibold text-zinc-900 dark:text-zinc-100">
        {title || t("s10_shutdown_step0_title")}
      </h2>
              ...
              <g key="shutdown">
                ...
                {step >= 2 && (
                  <DecisionBox
                    ...
                    approveLabel={t("s10_approve")}
                    rejectLabel={t("s10_reject")}
                  />
                )}
