...
const ruScenarioModules: Record<string, () => Promise<{ default: Scenario }>> = {
  ...
  s12: () => import("@/data/scenarios/s12.ru.json") as Promise<{ default: Scenario }>,
};
...
export function AgentLoopSimulator({ version }: AgentLoopSimulatorProps) {
  ...
  const ts = useTranslations("sim");
  const locale = useLocale();
  const [scenario, setScenario] = useState<Scenario | null>(null);
  const scrollRef = useRef<HTMLDivElement>(null);

  useEffect(() => {
    const loader = locale === "ru"
      ? ruScenarioModules[version] || scenarioModules[version]
      : scenarioModules[version];
    if (loader) {
  ...
  return (
    <section>
      ...
      {locale === "ru" && !ruScenarioModules[version] && (
