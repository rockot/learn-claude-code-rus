...
export default function ComparePage() {
  ...
  const localizedMetaA = metaA
    ? getLocalizedVersionMeta(versionA, tSession, tSubtitle, tCore, tKey)
    : null;
  const localizedMetaB = metaB
    ? getLocalizedVersionMeta(versionB, tSession, tSubtitle, tCore, tKey)
    : null;
          ...
          >
            ...
            {LEARNING_PATH.map((v) => (
              <option key={v} value={v}>
                {v} - {getLocalizedVersionMeta(v, tSession, tSubtitle, tCore, tKey).title}
              </option>
            ))}
          </select>
        </div>
      ...
      {infoA && infoB && comparison && (
        <div className="space-y-8">
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            <Card>
              <CardHeader>
                <CardTitle>{localizedMetaA?.title || versionA}</CardTitle>
                <p className="text-sm text-zinc-500">{localizedMetaA?.subtitle}</p>
              </CardHeader>
              <div className="space-y-2 text-sm text-zinc-600 dark:text-zinc-400">
                <p>{infoA.loc} {tv("loc_short")}</p>
                <p>{infoA.tools.length} {tv("tools")}</p>
                {metaA && (
                  <LayerBadge layer={metaA.layer}>
                    {getLocalizedLayerLabel(metaA.layer, tLayer)}
                  </LayerBadge>
                )}
              </div>
            </Card>
            <Card>
              <CardHeader>
                <CardTitle>{localizedMetaB?.title || versionB}</CardTitle>
                <p className="text-sm text-zinc-500">{localizedMetaB?.subtitle}</p>
              </CardHeader>
              <div className="space-y-2 text-sm text-zinc-600 dark:text-zinc-400">
                <p>{infoB.loc} {tv("loc_short")}</p>
                <p>{infoB.tools.length} {tv("tools")}</p>
                {metaB && (
                  <LayerBadge layer={metaB.layer}>
                    {getLocalizedLayerLabel(metaB.layer, tLayer)}
                  </LayerBadge>
                )}
              </div>
            </Card>
            ...
            <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
              <div>
                <h3 className="mb-3 text-sm font-medium text-zinc-500 dark:text-zinc-400">
                  {localizedMetaA?.title || versionA}
                </h3>
                <ArchDiagram version={versionA} />
              </div>
              <div>
                <h3 className="mb-3 text-sm font-medium text-zinc-500 dark:text-zinc-400">
                  {localizedMetaB?.title || versionB}
                </h3>
            ...
            <div className="grid grid-cols-1 gap-6 sm:grid-cols-3">
              <div>
                <h4 className="mb-2 text-sm font-medium text-zinc-600 dark:text-zinc-400">
                  {t("only_in")} {localizedMetaA?.title || versionA}
                </h4>
              ...
              <div>
                <h4 className="mb-2 text-sm font-medium text-zinc-600 dark:text-zinc-400">
                  {t("only_in")} {localizedMetaB?.title || versionB}
                </h4>
                {comparison.toolsOnlyB.length === 0 ? (
                  <p className="text-xs text-zinc-400">{t("none")}</p>
                ) : (
