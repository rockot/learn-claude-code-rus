...
import { getLocalizedVersionMeta } from "@/lib/version-meta";
...
export default async function VersionPage({
  ...
  const tSubtitle = getTranslations(locale, "subtitles");
  const tCore = getTranslations(locale, "core_additions");
  const tKey = getTranslations(locale, "key_insights");
  const tLayer = getTranslations(locale, "layer_labels");

  if (!versionData || !meta) {
    return (
      <div className="py-20 text-center">
        <h1 className="text-2xl font-bold">{t("version_not_found")}</h1>
        <p className="mt-2 text-zinc-500">{version}</p>
      </div>
    );
  }
  const layer = LAYERS.find((l) => l.id === meta.layer);
  const localizedMeta = getLocalizedVersionMeta(
    version,
    tSession,
    tSubtitle,
    tCore,
    ...
    <div className="mx-auto max-w-3xl space-y-10 py-4">
      ...
      <header className="space-y-3">
        <div className="flex flex-wrap items-center gap-3">
          ...
          <h1 className="text-2xl font-bold sm:text-3xl">{localizedMeta.title}</h1>
          {layer && (
          ...
          >
            ...
            <div>
              ...
              <div className="font-medium">
                {prevVersion} - {getLocalizedVersionMeta(prevVersion, tSession, tSubtitle, tCore, tKey).title}
              </div>
            </div>
          </Link>
        ) : (
        ...
        {nextVersion ? (
          ...
          >
            <div>
              ...
              <div className="font-medium">
                {getLocalizedVersionMeta(nextVersion, tSession, tSubtitle, tCore, tKey).title} - {nextVersion}
              </div>
            </div>
            <span className="transition-transform group-hover:translate-x-1">
              &rarr;
