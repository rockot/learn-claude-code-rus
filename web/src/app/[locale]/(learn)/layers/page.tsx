        ...
        {LAYERS.map((layer, index) => {
          const versionInfos = layer.versions.map((vId) => {
            ...
            const localizedMeta = meta
              ? getLocalizedVersionMeta(vId, tSession, tSubtitle, tCore, tKey)
              : null;
            return { id: vId, info, meta, localizedMeta };
          });
            ...
            >
              ...
              <div className="flex items-center gap-3 px-6 py-4">
                ...
                <div>
                  <h2 className="text-xl font-bold">
                    ...
                    {" "}
                    {getLocalizedLayerLabel(layer.id, tLayer)}
                  </h2>
              ...
              <div className="border-t border-zinc-200 bg-zinc-50/50 px-6 py-4 dark:border-zinc-800 dark:bg-zinc-900/50">
                <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
                  {versionInfos.map(({ id, info, meta, localizedMeta }) => (
                    <Link
                      key={id}
                      href={`/${locale}/${id}`}
                      className="group"
                    >
                      <Card className="transition-shadow hover:shadow-md">
                        <div className="flex items-start justify-between">
                          <div className="min-w-0 flex-1">
                            ...
                            <h3 className="mt-1 font-semibold text-zinc-900 dark:text-zinc-100">
                              {localizedMeta?.title || meta?.title || id}
                            </h3>
                        ...
                        <div className="mt-3 flex items-center gap-4 text-xs text-zinc-500 dark:text-zinc-400">
                          <span>{info?.loc ?? "?"} {tv("loc_short")}</span>
                          <span>{info?.tools.length ?? "?"} {tv("tools")}</span>
                        </div>
