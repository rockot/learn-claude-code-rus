...
// docs/rus/s01-kak-rabotaet-agent.md -> "ru"
function detectLocale(relPath: string): "en" | "zh" | "ja" | "ru" {
  ...
  if (relPath.startsWith("rus/") || relPath.startsWith("rus\\")) return "ru";
  return "en";
}

function localeDirToCode(localeDir: string): "en" | "zh" | "ja" | "ru" {
  return localeDir === "rus" ? "ru" : detectLocale(`${localeDir}/`);
}
...
function main() {
  ...
  // 4. Read doc files from locale subdirectories (en/, zh/, ja/, rus/)
  const docs: DocContent[] = [];

  if (fs.existsSync(DOCS_DIR)) {
    const localeDirs = ["en", "zh", "ja", "rus"];
    let totalDocFiles = 0;

    for (const localeDirName of localeDirs) {
      const locale = localeDirToCode(localeDirName);
      const localeDir = path.join(DOCS_DIR, localeDirName);
      ...
      for (const filename of docFiles) {
        ...
        if (!version) {
          console.warn(`  Skipping doc ${localeDirName}/${filename}: could not determine version`);
          continue;
        }
